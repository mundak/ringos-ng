...
namespace
{
  ...
  int32_t capture_x64_syscall(void* context, const x64_emulator_state& state, bool* out_should_continue)
  {
    ...
    const uint64_t argument0 = state.general_registers[static_cast<uint32_t>(X64_GENERAL_REGISTER_RDI)];
  ...
  uint64_t initial_rax)
{
  ...
  state.flags = 0x202;
  state.general_registers[static_cast<uint32_t>(X64_GENERAL_REGISTER_RAX)] = initial_rax;
  state.general_registers[static_cast<uint32_t>(X64_GENERAL_REGISTER_RSP)]
