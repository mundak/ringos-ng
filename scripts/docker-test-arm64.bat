...
docker run --rm %IMAGE_NAME% bash -lc "cmake --preset arm64-debug && cmake --build --preset build-arm64-debug && ctest --preset smoke_arm64_native && ctest --preset smoke_arm64_ansi_c && ctest --preset smoke_arm64_x64_emulator"
if %errorlevel% neq 0 (
