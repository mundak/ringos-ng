...
docker run --rm -it %TOOLCHAIN_TOKEN_ARG% -v "%HOST_BUILD_DIR%:/workspace/build" %IMAGE_NAME% bash -lc "bash tools/toolchain/download-latest-toolchain.sh --repo mundak/ringos-ng && cmake --preset arm64-debug && cmake --build --preset build-arm64-debug && scripts/run-arm64.sh build/arm64-debug/arch/arm64/ringos_arm64"
if %errorlevel% neq 0 (
