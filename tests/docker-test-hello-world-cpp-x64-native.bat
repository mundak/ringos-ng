...
docker run --rm %TOOLCHAIN_TOKEN_ARG% -v "%HOST_BUILD_DIR%:/workspace/build" %IMAGE_NAME% bash -lc "bash tools/toolchain/download-latest-toolchain.sh --repo mundak/ringos-ng && cmake --preset x64-debug && cmake --build --preset build-x64-debug --target ringos_x64_hello_world_cpp && ctest --preset sample_hello_world_cpp_x64_native"
if %errorlevel% neq 0 (
