#!/usr/bin/env bash
# Smoke test for the x64 kernel image that boots the hello_world sample.
...
for expected_line in \
  ...
  if ! grep -Fq -- "${expected_line}" "${DEBUG_LOG}"; then
    echo "FAIL: expected '${expected_line}' not found in x64 hello_world debug output" >&2
    echo "--- debug output ---" >&2
    cat "${DEBUG_LOG}" >&2
    exit 1
  fi
...
echo "PASS: x64 hello_world sample test"
