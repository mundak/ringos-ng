...
docker run --rm %TOOLCHAIN_TOKEN_ARG% -v "%HOST_RINGOS_CACHE%:/root/.cache/ringos" %IMAGE_NAME% bash -lc "bash tools/toolchain/ensure-toolchain-release.sh --repo mundak/ringos-ng --allow-build && cmake --preset x64-debug && cmake --build --preset build-x64-debug --target ringos_x64 && ctest --preset sample_hello_world_x64_native"
if %errorlevel% neq 0 (
