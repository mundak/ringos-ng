...
docker run --rm %TOOLCHAIN_TOKEN_ARG% -v "%HOST_BUILD_DIR%:/workspace/build" %IMAGE_NAME% bash -lc "bash tools/toolchain/download-latest-toolchain.sh --repo mundak/ringos-ng && cmake --preset arm64-debug && cmake --build --preset build-arm64-debug --target ringos_arm64_console_service_write && ctest --preset sample_console_service_write_arm64_native"
if %errorlevel% neq 0 (
