#!/usr/bin/env bash
# Smoke test for the arm64 kernel image that boots the x64 hello_world sample through the emulator.
...
for expected_line in \
  ...
  if ! grep -Fq -- "${expected_line}" "${DEBUG_LOG}"; then
    echo "FAIL: expected '${expected_line}' not found in arm64 x64-emulator hello_world debug output" >&2
    echo "--- debug output ---" >&2
    cat "${DEBUG_LOG}" >&2
    exit 1
  fi
...
echo "PASS: arm64 x64-emulator hello_world sample test"
