...
```bash
cmake -S user/samples/console_service_write -B build/user-samples/console_service_write-x64 -G Ninja \
  -DCMAKE_TOOLCHAIN_FILE="$HOME/.cache/ringos/toolchain/cmake/ringos-toolchain.cmake" \
  -DRINGOS_TARGET_ARCH=x64
cmake --build build/user-samples/console_service_write-x64
```
